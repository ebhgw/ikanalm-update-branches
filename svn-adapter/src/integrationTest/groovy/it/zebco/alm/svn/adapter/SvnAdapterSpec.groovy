...
@Testcontainers
class SvnAdapterSpec extends Specification {
    ...
    def "Check svn ls on an existing directory" (String path, boolean res) {
        given:
        SvnAdapter sa = new SvnAdapter(new SvnLsCommandImpl(this.repoRootUrl, this.username, this.password),
                new SvnmuccMkdirsCommandImpl(this.repoRootUrl, this.username, this.password))
        expect:
        sa.exists(path) == res
        where:
        path | res
    ...
    def "Check svn ls raising errors" () {
        ...
        SvnAdapter sa = new SvnAdapter(new SvnLsCommandImpl(this.repoRootUrl, 'unknown', 'unknown'),
                new SvnmuccMkdirsCommandImpl(this.repoRootUrl, 'unknown', 'unknown'))
        when:
        sa.exists('branches/correttiva')
        then:
    ...
    def "Create some directories" (String path, String res) {
        ...
        SvnAdapter sa = new SvnAdapter(new SvnLsCommandImpl(this.repoRootUrl, this.username, this.password),
                new SvnmuccMkdirsCommandImpl(this.repoRootUrl, this.username, this.password))
        expect:
        sa.mkDirs('branches', path) == res && checkHelper(res) == check
        where:
        path|res|check
    ...
    def "Put a file via svnmucc" (Map info, String path, int exit) {
        given:
        SvnAdapter sa = new SvnAdapter(new SvnLsCommandImpl(this.repoRootUrl, this.username, this.password),
                new SvnmuccPutCommandImpl(this.repoRootUrl, this.username, this.password))
        SvnmuccPutCommandImpl mucc = new SvnmuccPutCommandImpl()
        File temp = File.createTempFile("tmp", ".txt");
        temp.deleteOnExit();
        temp << "text file"
        expect:
        sa.put(info, temp, path) == exit
        where:
        info|path|exit

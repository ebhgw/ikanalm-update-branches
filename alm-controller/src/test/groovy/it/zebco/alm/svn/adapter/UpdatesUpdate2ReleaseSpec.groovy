...
@Slf4j
class UpdatesUpdate2ReleaseSpec extends Specification {
    ...
    def setup () {
        ...
        upds = new Updates(cr: cr, muccBlder: muccBlder)
    }

    def "update2Release branch empty list"() {
        when:
        def res = upds.update2Release('http://host/repo/proj/branch/correttiva', [], "commit")
        then:
        thrown(AssertionError)
    }

    def "update2Release branch don't found files"() {
        given:
        ...
        // print stream aggiunge un \r\n
        System.out = new PrintStream(buffer)
        def res = upds.update2Release('http://host/repo/proj/branches/correttiva', lsi, "commit")
        then:
        thrown(AssertionError)
    }

    def "update2Release branch found file"() {
        given:
        File working = testDir.newFolder('source')
        File di1 = testDir.newFolder('source', 'batch')
        ...
        def res = upds.update2Release('http://host/repo/proj/branches/correttiva', lsi, "commit")
        then:
        executeCommandBuffer.toString() == 'svnmucc -U http://host/repo/proj/branches/correttiva ' +
                'put batch\\test.cbl batch/test.cbl ' +
                'propset almProjectName SS01OTX batch/test.cbl ' +
                'propset almProjectStreamBuildPrefix SIGE1801 batch/test.cbl ' +
                'propset almProjectStreamBuildSuffix R02 batch/test.cbl ' +
                '-u user -p password --no-auth-cache -m "commit"'
        //note file batch/test.cbl and batch\test.cbl both are equally accepted
    }

    def "update2Release branch found several files"() {
        given:
        File working = testDir.newFolder('source')
        File di1 = testDir.newFolder('source', 'batch')
        File fi1 = new File(di1, 'test.cbl')
        ...
        executeCommandBuffer.toString() == 'svnmucc -U http://host/repo/proj/branches/correttiva ' +
                ...
                'propset almProjectStreamBuildPrefix SIGE1801 batch/test.cbl ' +
                'propset almProjectStreamBuildSuffix R02 batch/test.cbl ' +
                'put cics\\test.cbl cics/test.cbl ' +
                'propset almProjectName SS01OTX cics/test.cbl ' +
                'propset almProjectStreamBuildPrefix SIGE1801 cics/test.cbl ' +
                'propset almProjectStreamBuildSuffix R02 cics/test.cbl ' +
                '-u user -p password --no-auth-cache -m "commit"'
        //note file batch/test.cbl and batch\test.cbl both are equally accepted
    }

    def "update2Release branch with -f option"() {
        given:
        File working = testDir.newFolder('source')
        File di1 = testDir.newFolder('source', 'batch')
        File fi1 = new File(di1, 'test.cbl')
        ...
        def filePathMatcher = generatedCommand.toString() =~ /-f (.*(temp.*\.tmp))/
        File tmpFile = new File(filePathMatcher[0][1])
        List<String> fileContent = tmpFile.readLines()
        then:
        generatedCommand.toString().take(55) == 'svnmucc -U http://host/repo/proj/branches/correttiva -f'
        fileContent[0..14] == ['put', 'batch\\test0.cbl', 'batch/test0.cbl', 'propset', 'almProjectName',
                               'SS01OTX', 'batch/test0.cbl', 'propset', 'almProjectStreamBuildPrefix', 'SIGE1801', 'batch/test0.cbl',
                               'propset', 'almProjectStreamBuildSuffix', 'R02', 'batch/test0.cbl']
        fileContent[1500..1514] == ['put', 'batch\\test100.cbl', 'batch/test100.cbl', 'propset', 'almProjectName',
                                    'SS01OTX', 'batch/test100.cbl', 'propset', 'almProjectStreamBuildPrefix', 'SIGE1801', 'batch/test100.cbl',
                                    'propset', 'almProjectStreamBuildSuffix', 'R02', 'batch/test100.cbl']
        generatedCommand.drop(generatedCommand.toString().size() - 47) == '-u user -p password --no-auth-cache -m "commit"'
        //buffer.toString() == 'Command: svnmucc -U http://host/repo/proj/branches/correttiva put batch\\test.cbl batch/test.cbl put cics\\test.cbl cics/test.cbl -u user -p password --no-auth-cache -m "commit"'
        //note file batch/test.cbl and batch\test.cbl both are equally accepted
    }
}

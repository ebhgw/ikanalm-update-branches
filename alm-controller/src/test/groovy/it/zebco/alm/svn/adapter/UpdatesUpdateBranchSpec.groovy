...
@Slf4j
class UpdatesUpdateBranchSpec extends Specification {
    ...
    def "update branch don't found files"() {
        given:
        ...
        def res = upds.update('http://host/repo/proj/branches/correttiva', lsi, "commit")
        then:
        thrown(AssertionError)
    }

    def "update branch found files"() {
        given:
        File working = testDir.newFolder('source')
        File di1 = testDir.newFolder('source', 'batch')
        ...
        def res = upds.update('http://host/repo/proj/branches/correttiva', lsi, "commit")
        then:
        executeCommandBuffer.toString() == 'svnmucc -U http://host/repo/proj/branches/correttiva put batch\\test.cbl batch/test.cbl -u user -p password --no-auth-cache -m "commit"'
        //note file batch/test.cbl and batch\test.cbl both are equally accepted
    }

    def "update branch found several files"() {
        given:
        File working = testDir.newFolder('source')
        File di1 = testDir.newFolder('source', 'batch')
        File fi1 = new File(di1, 'test.cbl')
        ...
        def res = upds.update('http://host/repo/proj/branches/correttiva', lsi, "commit")
        then:
        executeCommandBuffer.toString() == 'svnmucc -U http://host/repo/proj/branches/correttiva put batch\\test.cbl batch/test.cbl put cics\\test.cbl cics/test.cbl -u user -p password --no-auth-cache -m "commit"'
        //        //note file batch/test.cbl and batch\test.cbl both are equally accepted
    }

    def "update branch with -f option"() {
        given:
        File working = testDir.newFolder('source')
        File di1 = testDir.newFolder('source', 'batch')
        File fi1 = new File(di1, 'test.cbl')
        ...
        fileContent[0..2] == ['put', 'batch\\test0.cbl', 'batch/test0.cbl']
        fileContent[300..302] == ['put','batch\\test100.cbl','batch/test100.cbl']
